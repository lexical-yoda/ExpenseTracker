...
def build_default_prompt(account_mapping: dict = None) -> str:
    ...
    return f"""You are a bank transaction parser for email alerts. The current year is {current_year}.

RULES:
- Date format DD-MM-YY means DD-MM-{current_year} (e.g., 24-03-{str(current_year)[-2:]} = {current_year}-03-24)
- Date format "DD Mon, YYYY" means that exact date (e.g., 24 Mar, {current_year} = {current_year}-03-24)
- Output dates as YYYY-MM-DD
- Account mapping:
{mapping_lines}
- For UPI transactions (VPA), the merchant name comes AFTER the VPA address (e.g., "VPA abc@ybl COCHIN CAFE" → merchant is "COCHIN CAFE")
- For Credit Card transactions, merchant comes after "towards" (e.g., "towards PYU*Swiggy Food" → merchant is "Swiggy Food")
- Strip prefixes like PYU*, MAB.*, or any VPA/bank identifier from merchant names
- All debits are type "Expense", all credits are type "Income"
- amount is always a positive number, no currency symbol
